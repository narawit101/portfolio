...
const TechStackTile = ({ item }: { item: TechStackItem }) => (
    <div className="theme-panel theme-panel-hover flex flex-col items-center gap-2 rounded-2xl px-3 py-4 text-center transition-all">
        ...
        <span className="text-[11px] font-semibold tracking-wide text-(--theme-text)">{item.name}</span>
    </div>
)

const ITEMS_PER_PAGE = 8

const TechStackCategoryCard = ({ category, lang }: { category: TechStackCategory; lang: Lang }) => {
        ...
        <article className="theme-card theme-card-hover relative overflow-hidden rounded-3xl transition-transform duration-500 hover:-translate-y-2">
            ...
            <div className="relative p-6 sm:p-8 space-y-4">
                <div className="flex items-center justify-between">
                    <span className="theme-text-muted text-xs uppercase tracking-[0.15em]">{category.title[lang]}</span>
                </div>
                <p className="theme-text-body text-sm leading-relaxed">{category.description[lang]}</p>
                <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
                    {visibleItems.map((item) => (
                        <TechStackTile key={`${category.title.en}-${item.name}-${currentPage}`} item={item} />
                    ))}
                </div>
                {hasPagination && (
                    <PaginationNav
        ...
        <section id="section-stack" className="scroll-mt-20 py-12 lg:py-14 px-6 sm:px-10 relative">
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                <div className="text-center mb-12">
                    ...
                    <p className="text-xs tracking-[0.35em] text-primary uppercase">{copy.subtitle}</p>
                    <div className="mt-4 h-[3px] w-12 bg-primary rounded-full mx-auto" />
                </div>

                <div className="grid gap-6 md:grid-cols-2">
                    {techStackCategories.map((category) => (
                        <TechStackCategoryCard key={category.title.en} category={category} lang={lang} />
                    ))}
                </div>
            </div>
        </section>
